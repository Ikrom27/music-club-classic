...
import com.ikrom.innertube.models.MusicCardShelfRenderer
            ...
            return when {
                renderer.onTap.watchEndpoint != null -> {
                    SongItem(
                        ...
                        itemArtists = subtitle?.getOrNull(1)?.oddElements()?.map {
                            ItemArtist(
                                name = it.text,
                                id = it.navigationEndpoint?.browseEndpoint?.browseId
                            )
                        } ?: return null,
                        itemAlbum = subtitle.getOrNull(2)?.firstOrNull()?.takeIf { it.navigationEndpoint?.browseEndpoint != null }?.let {
                            ItemAlbum(
                                name = it.text,
                                id = it.navigationEndpoint?.browseEndpoint?.browseId!!
                            )
            ...
            return when {
                renderer.isSong -> {
                    SongItem(
                        ...
                        itemAlbum = secondaryLine.getOrNull(2)?.firstOrNull()?.takeIf { it.navigationEndpoint?.browseEndpoint != null }?.let {
                            ItemAlbum(
                                name = it.text,
                                id = it.navigationEndpoint?.browseEndpoint?.browseId!!
                            )
