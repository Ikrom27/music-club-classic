...
class MainFragment: Fragment() {
    ...
    private fun bindMiniPlayer(){
        viewModel.currentMediaItem.observe(viewLifecycleOwner) {
            if (it != null){
                miniPlayer.title = it.mediaMetadata.title.toString()
                miniPlayer.subTitle = it.mediaMetadata.artist.toString()
                miniPlayer.thumbnailUrl = it.mediaMetadata.artworkUri?.toString() ?: ""
            }
        }
        viewModel.isPlaying.observe(viewLifecycleOwner) {
            miniPlayer.btnIcon = if (it) AppIconsId.pause else AppIconsId.play
        }
    }
